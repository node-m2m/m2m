...
const  {defaultNode, emitter, m2mUtil, m2mStart, m2mRestart, setAttributes, rid} = require('./client.js');

const processArgs = process.argv.slice(2);
var m2m = {}, remoteServer = null, clientArgs = [];
...
function accessDevice(){
  ...
  try{
    ...
    if(arguments.length > 1 && typeof arguments[0] === 'number' && typeof arguments[1] === 'number'){
      ...
      throw new Error('more than 1 argument must be contained in an array');
    }
    ...
    // async accessDevice  
    // cb only, using clientArgs, e.g. client.accessDevice(cb)
    if(arguments.length === 1 && typeof arguments[0] === 'function'){
      if(clientServer.length === 0){
        throw new Error('missing device/server arguments');    
      }
      cb = arguments[0];
      if(clientServer.length > 1 ){
        if(cb){
          return setImmediate(cb, null, clientServer);
        }
      }
      else {
        if(cb){
          return setImmediate(cb, null, clientServer[0]);
        }
      }
    }
...
const Client = exports.Client = function(){
  ...
  if(arguments.length === 1 && typeof arguments[0] === 'object'){
    if(arguments[0].client && typeof arguments[0].client === 'object'){
      m2m.options = arguments[0].client;
    }
    else{
    ...
    if(arguments[0].server && Array.isArray(arguments[0].server)){
      clientArgs = arguments[0].server;
      m2m.server = clientArgs;
    }
  }
...
function connect(args, cb){
  ...
  // userid and password are provided as options
  if(args && typeof args === 'object' && args.userid && args.pw){
    //return m2mStart(m2m, args, cb);
    return m2mRestart(args, m2m, cb);
  }

  // register user if token is available
  // if token is not available or valid, user will be prompted for credentials 
  if(args && !args.userid && !args.pw || (!args)){
