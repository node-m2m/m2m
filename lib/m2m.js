...
/**
 * 	M2M Server Endpoint (implicit cloud api)
 *
...
/**
 * M2M Client Endpoint (implicit cloud api)
 *  
...
/**
 * 	M2M Cloud Endpoint (explicit cloud api)
 *
 * 	usage e.g. 	
 *
 *	const cloud = new m2m.Cloud();
 *	let cs = new cloud.Server(500);	  	
 *
 *	let cc = new Cloud({type:'client', name:'cloud-client'})
 *	let cs = new Cloud({type:'server', id:300}) or new Cloud({type:'server', port:300}) where id or port are considered as virtual port 
 */
m2m.Cloud = function CloudEndpoint(arg){
	if(arg && arg.type === 'client'){
		return m2m.Client(arg);
	}
	else if(arg && arg.type === 'server' && arg.port){
		return m2m.Server(arg); 
	}
	else if(arg && arg.type === 'server' && arg.id){
		return m2m.Server(arg);  
	}
}
