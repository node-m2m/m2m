  ...
  ************************************/
  function rxEpClientData(rxd){
    ...
    try{
      ...
      if(rxd.name){ //rxd.topic||rxd.channelrxd.api
...
m2mLib.server = () => {
  ...
  ********************************************/
  ...
  function serverClient(){
    const { read, write, subscribe, sub, unsubscribe, unsub, get, post } = m2mLib.client();
    return { read, write, subscribe, sub, unsubscribe, unsub, get, post };
  }
  ...
  function rxEpServerData(rxd){
    try{
      if(rxd.server && rxd.dstId && rxd.error){
        ...
        //if(rxd.watch && rxd.event){ // valid only for m2m client
        if(rxd.watch && rxd.event && rxd.dst !== 'server-as-client'){ // valid also for server-as-client 5/2/24
          return watchChannelData(rxd);
        }
        //else if(rxd.unwatch){ // valid only for m2m client
        else if(rxd.unwatch && rxd.dst !== 'server-as-client'){ // valid also for server-as-client 5/2/24
