...
const dm = epService.dm = (() => {
  ...
  let publishDataRate = [], dataSourceDataRate = [], httpGetRate = [], httpPostRate = [];
  ...
  function resetTotalCount(){
    publishDataRate = [], dataSourceDataRate = [], httpGetRate = [], httpPostRate = [];
  ...
  function initDeviceData(arrayData, accessRate){
    arrayData.forEach((name, x) => {
  ...
  function monitorAccessRate(){
    ...
    let publishData = deviceResources.publish.name; 
    let dataSourceData = deviceResources.dataSource.name;
    let getPathData = deviceResources.httpApi.getPath;
    let postPathData = deviceResources.httpApi.postPath;
    let inputPin = deviceResources.gpio.input.pin;
    let outputPin = deviceResources.gpio.output.pin;

    if(monAccessRateCounter === 1){
      initDeviceData(publishData, publishDataRate);
      initDeviceData(dataSourceData, dataSourceDataRate);

      initDeviceData(getPathData, httpGetRate);
      initDeviceData(postPathData, httpPostRate);
    ...
    monitorAccessRateTimeout = setTimeout(() => {
      ...
      resourceAccessRateContainer.forEach((arData, x) => {
        try{
          if(arData.name && arData.watch && arData.method === 'watch'){
            publishDataRate.forEach((o, x) => {
              if(o.name === arData.name){
                o.cpm++;
                o.watch = true;
              }            
            });
          }
          else if(arData.name && arData.getData && arData.method === 'getData'){
            dataSourceDataRate.forEach((o, x) => {
              if(o.name === arData.name){
                o.cpm++;
                o.getData = true;
              }            
          ...
          else if(arData.name && arData.sendData && arData.method === 'sendData'){
            dataSourceDataRate.forEach((o, x) => {
              if(o.name === arData.name){
                o.cpm++;
                o.sendData = true;
          ...
          else if(arData.name && arData.get && arData.method === 'get'){
            httpGetRate.forEach((o, x) => {
              if(o.name === arData.name||o.name === arData.baseUrl){
                o.cpm++;
                o.get = true;
              }            
            });
          }
          else if(arData.name && arData.post && arData.method === 'post'){
            httpPostRate.forEach((o, x) => {
              if(o.name === arData.name||o.name === arData.baseUrl){
                o.cpm++;
                o.post = true;
              }            
          ...
          else if(arData.gpioInput && arData.watch && arData.pin){
            inputWatchRate.forEach((o, x) => {
              if(o.pin === arData.pin){
                o.cpm++;
                o.input = true;
                o.watch = true;
          ...
          else if(arData.gpioInput && arData.getState && arData.pin){
            inputGetStateRate.forEach((o, x) => {
              if(o.pin === arData.pin){
                o.cpm++;
                o.input = true;
                o.getState = true;
          ...
          else if(arData.gpioOutput && arData.on && arData.pin){
            outputOnRate.forEach((o, x) => {
              if(o.pin === arData.pin){
                o.cpm++;
                o.output = true;
                o.on = true;
          ...
          else if(arData.gpioOutput && arData.off && arData.pin){
            outputOffRate.forEach((o, x) => {
              if(o.pin === arData.pin){
                o.cpm++;
                o.output = true;
                o.off = true;
      ...
      setAccessCount(publishDataRate); setAccessCount(dataSourceDataRate);
      setAccessCount(httpGetRate); setAccessCount(httpPostRate);
      setAccessCount(inputWatchRate); setAccessCount(inputGetStateRate);
      setAccessCount(outputOnRate); setAccessCount(outputOffRate);
   ...
   ******************************/
  let clientRequestResourcesData = [];

  function setDeviceAccess(args){
    let match = false;
    if(args.unwatch){
      return;
    }
    let arrayData = clientAccessData;
      ...
      setImmediate(() => {
        for (let i = 0; i < arrayData.length; i++ ) {
          if(arrayData[i]){
            ...
            if(arrayData[i].name && arrayData[i].sendData === args.sendData && arrayData[i].id === args.id && arrayData[i].name === args.name){
              match = true;
              break;  
            }
  ...
  function validateClientAccessData(){
    let newArray = [];
    let validDeviceId = client.device.getValidDeviceId(); // validate device id
    if(clientAccessData.length > 0){
      for (let i = 0; i < clientAccessData.length; i++ ) {
        if(clientAccessData[i]){
          if(validDeviceId.includes(clientAccessData[i].id)){
            newArray.push(clientAccessData[i]);
          }
        }
      }
    }
  ...
  function setClientRequestResourcesData(args){
    let match = false, ip = m2mUtil.systemConfig.systemInfo.ip; 
   
    let clientObject = {id:args.id, appId:args.appId, ip:ip, clientSub:[], clientRead:[], clientWrite:[], clientGet:[], clientPost:[],
    clientInputGetState:[], clientInputWatch:[], clientOutputGetState:[], clientOutputOn:[], clientOutputOff:[]};

    clientRequestResourcesData.forEach((item, x) => {
      if(item.id === args.id){
        match = true;
      } 
    });
    if(match === false){
      clientRequestResourcesData.push(clientObject);
    }
  }

  function processAccessData(array, data){
    array.push(data);
    return m2mUtil.removeDuplicateInArray(array);
  }

  function processRequestResourcesData(){
    validateClientAccessData();
    clientAccessData.forEach((args) => {
      setClientRequestResourcesData(args);
    });
    clientRequestResourcesData.forEach((item, x) => {
      clientAccessData.forEach((args, i) => {   
        if(args.id === item.id){
          if(args.getData && args.name){
            item.clientRead = processAccessData(item.clientRead, args.name);
          }
          else if(args.sendData && args.name){
            item.clientWrite = processAccessData(item.clientWrite, args.name);
          }
          else if(args.watch && args.name){
            item.clientSub = processAccessData(item.clientSub, args.name);
          }
          else if(args.get && args.name){
            item.clientGet = processAccessData(item.clientGet, args.name);
          }
          else if(args.post && args.name){
            item.clientPost = processAccessData(item.clientPost, args.name);
          }
          else if(args.gpioInput && args.getState && args.pin){
            item.clientInputGetState = processAccessData(item.clientInputGetState, args.pin);
          }
          else if(args.gpioInput && args.watch && args.pin){
            item.clientInputWatch = processAccessData(item.clientInputWatch, args.pin);    
          }
          else if(args.gpioOutput && args.output === 'state' && args.pin){
            item.clientOutputGetState = processAccessData(item.clientOutputGetState, args.pin);    
          }
          else if(args.gpioOutput && args.on && args.pin){
            item.clientOutputOn = processAccessData(item.clientOutputOn, args.pin);  
          }
          else if(args.gpioOutput && args.off && args.pin){
            item.clientOutputOff = processAccessData(item.clientOutputOff, args.pin);  
          }
        }
      }); 
    });
    return clientRequestResourcesData;
  }
  
  function getClientAccessData(){
    return processRequestResourcesData();
  } 
 ...
 ****************************************/
/* istanbul ignore next */
const m2mUtil = epService.m2mUtil = (() => {
  ...
  function m2mInfo(pl, server){
    ...
    if(pl.device){
      ...
      if(pl.userId){
        console.log('   userId:', pl.userId);
      } 
    }
    else{
      console.log('   Endpoint authentication');
    }  
    console.log('   Server:', colors.brightBlue(server), '\n***************************************');
  }  
  ...
  function setToBuffer(arg, type){
    if(type === 'base64'){
        return Buffer.from(JSON.stringify(arg)).toString('base64');
    }
    else if(type === 'hex'){
        return Buffer.from(JSON.stringify(arg)).toString('hex');
    }
  }
  ...
  function removeWatchFile(filename, arrayData){
    if(arrayData.length > 0){
      for (let i = 0; i < arrayData.length; i++ ) {
  ...
  function checkWatchFile(filename, arrayData){
    if(arrayData.length > 0){
      for (let i = 0; i < arrayData.length; i++ ) {
        if(arrayData[i] && arrayData[i].filename && filename && arrayData[i].filename === filename){
          return true;
  ...
  function monExtFileSync(o){
    ...
    function setFileWatcher(){
      try{
        ...
        if(e){
          //console.log('setFileWatcher fs.watch error', e.message);
        }
      }
      fileWatcher.on('close', () => {
        // alternate re-watch file method
      ...
      fileWatcher.on('error', (err) => {
        console.log('setFileWatcher fileWatcher event error', err.message);
      });
    }
 ...
 ********************************************/
const edge = epService.edge = (() => {
  const Edge = require('node-edge');
 
  function startCreateClient(arg){
    return Edge.createClient;
  }

  function startCreateServer(arg){
    return Edge.createServer;
  }

  function epResources(rxd){
    if(rxd.id === spl.id){
  ...
  function edgeConnect(arg){
    arg.edge = true;
    let pl = m2mUtil.secTkn.setToBuffer(arg, 'hex');
    return Edge.connect(pl); 
  }  

  let ep = {
    access:accessEdge,
    resources:epResources,
  }

  return {
    endpoint:ep,
    edgeConnect: edgeConnect,
    createClient: Edge.createClient,
    createServer: Edge.createServer,
    startCreateClient: startCreateClient,
    startCreateServer: startCreateServer,
  }
})(); // edge
...
const client = epService.client = (() => {
  ...
  let channelAccessRate = 0, inputGpioAccessRate = 0, outputGpioAccessRate = 0, rateLimited = false;
  ...
  setTimeout(()=> {
    ...
    if(inputGpioAccessRate > 5){
      rateLimited = true;  
    }
    if(outputGpioAccessRate > 5){ 
      rateLimited = true;  
    }
  }, 5100) 
  ...
  *****************************************/
  ...
  const inputState = function(cb){
    ...
    let id = this.id, pin = this.pin
    if(cb){
        setGpioInputPayload(id, pin, 'getState', null, cb);
    }
    else{
        return new Promise(function (resolve, reject) { 
           setGpioInputPayload(id, pin, 'getState', null, (data) => {
              resolve(data)
  ...
  const unwatchState = function(cb){
    ...
    let id = this.id, pin = this.pin
    if(cb){
        setGpioInputPayload(id, pin, 'unwatch', null, cb);
    }
    else{
        return new Promise(function (resolve, reject) { 
           setGpioInputPayload(id, pin, 'unwatch', null, (data) => {
              resolve(data)
           })
        })
    }  
  ...
  ******************************************/
  ...
  const outputState = function(cb){
    ...
    let id = this.id, pin = this.pin
    if(cb){
        setGpioOutputPayload(id, pin, 'state', null, cb);
    }
    else{
        return new Promise(function (resolve, reject) { 
           setGpioOutputPayload(id, pin, 'state', null, (data) => {
              resolve(data)
           })
        })
    }  
  ...
  Output.prototype = {
    ...
    subscribe: invalidOutputMethod,
    unsubscibe: invalidOutputMethod,
    sub: invalidOutputMethod,
    unsub: invalidOutputMethod,
  ...
  ****************************************************/
  function setChannelEventListener(pl, cb){
    ...
    if(pl.event && pl.watch){
      // only 1 listener is allowed for the above eventName
      if(emitter.listenerCount(eventName) < 1){
    ...
    **************************************/
    ...
    urlRoutesIndex.forEach((value, i) => {
      ...
      // other url routes
        ...
        //let key = url.slice(urlRoutesIndex[i]+1, urlRoutesIndex[i+1]);    // no slash
        let key = url.slice(urlRoutesIndex[i], urlRoutesIndex[i+1]);
        urlKeys.push(key); 
      }
      else{
        //let key = url.slice(urlRoutesIndex[i]+1, url.length);
        let key = url.slice(urlRoutesIndex[i], url.length);
        urlKeys.push(key);
      }
    });
    ...
    try{
      ...
      if(typeof id === 'object' || typeof name === 'object'){
        ...
        if(method === 'get'){
          args.name = name;
          args.path = name;
          args.originalUrl = name;
          getUrlKeys(args, name);
        }
        if(method === 'post'){
          args.name = name;
          args.path = name;
  ...
  *******************************************/
  ...
  // get channel data - promise base api 9/8/23 e.g await device.read(topic, pl) or device.read(topic, pl, cb)
  const ChannelGetData = function(channel, cb){
    ...
    if(cb){
        ...
        return new Promise(function (resolve, reject) { 
           setClientDataPayload(id, channel, null, 'getData', null, (data) => {
              resolve(data)
           })
        })
    }  
  ...
  // send channel data - promise base api 9/8/23 e.g await device.read(topic) or device.read(topic, cb)
  const ChannelSendData = function(channel, payload, cb){
    ...
    if(cb){
        ...
        return new Promise(function (resolve, reject) { 
           setClientDataPayload(id, channel, payload, 'sendData', null, (data) => {
              resolve(data)
           })
        })
    }  
  ...
  const ChannelUnwatchData = function(channel, cb){
    ...
    if(cb){
        ...
        return new Promise(function (resolve, reject) { 
           setClientDataPayload(id, channel, null, 'unwatch', null, (data) => {
              resolve(data)
           })
        })
    }  
  ...
  deviceAccess.prototype = {
    ...
    // e.g. device.unwatch
    unsub: ChannelUnwatchData,
    subscribe: ChannelUnwatchData,
    ...
    // e.g. device.watch
    sub: ChannelWatchData,
    subscribe: ChannelWatchData,
  ...
  *************************************/
  ...
  // gpio input
  // e.g client.input(200, 13).watch(cb) or client.input(200, 13).subscribe(cb) or client.input({id:200, pin:33}).watch(cb)
  // client.input(200, 13).getState(cb) or client.input(200, 13).read(cb) 
  function input(id, pin){
  ...
  // client.output(200, 33).getState(cb) or client.output(200, 33).read(cb)
  function output(id, pin){
    validateApiDeviceId(id);
 ...
 ********************************************/
const device = epService.device = (() => {
  let randomNumber = Math.floor((Math.random() * 5000) + 1000);
  let deviceSetup = {id: spl.id, systemInfo: m2mUtil.systemConfig.systemInfo, gpio:{input:{pin:[]}, output:{pin:[]}}, httpApi:{}, dataSource:{name:[]}, publish:{name:[]}};
  ...
  let browserWatch = {reset:null, timeout:30*60000+randomNumber};  
  ...
  function resetSystem(){
    deviceSetup = {id: spl.id, systemInfo: m2mUtil.systemConfig.systemInfo, gpio:{input:{pin:[]}, output:{pin:[]}}, httpApi:{}, channel:{name:[]}, dataSource:{name:[]}};
  ...
  ***************************************/
  ...
  function watchChannelData(rxd){
    ...
    if(rxd.src === 'browser' && rxd.b){
      setTimeout( ()=> {
        browserWatch.reset(rxd);    
      }, browserWatch.timeout);
    }
    ...
    if(rxd.src === 'browser' && rxd.b){
      browserWatch.reset = function(rxd){
        rxd.unwatch = true;
        //removeWatchEvent(rxd, dataObject);
  ...
  ******************************************/
  ...
  function setDeviceResourcesData(args){
    process.nextTick(function(){
      ...
      if(typeof args === 'string'){
        ...
        if(typeof args.channel === 'string'||typeof args.name === 'string'){
          if(args.channel !== 'sec-cli'){
            if(args.method === 'dataSource'){
                deviceSetup.dataSource.name.push(args.channel);
            }
            else if(args.method === 'publish'){
                deviceSetup.publish.name.push(args.channel);
            }
          }
        }
        else if(Array.isArray(args.pin)){
        ...
        else if(simInputPin > 0){
          deviceSetup.gpio.input.type = 'simulation';
        }
		else if(rpiGpioInput > 0 && (os.arch() === 'arm' || os.arch() === 'arm64')){
          deviceSetup.gpio.input.type = 'rpi';
        }
        else if(simOutputPin > 0){
          deviceSetup.gpio.output.type = 'simulation';
        }
        else if(rpiGpioOutput > 0 && (os.arch() === 'arm' || os.arch() === 'arm64')){
  ...
  function setEmitterOutput(data, cb){
    if(data.error){
      try{   
        emitter.emit('error', data);
      }
        ...
        if(emitter.listenerCount(EventName) < 1){
          emitter.on(EventName, (data) => {
            ...
            if(data.id === spl.id && data.pin === pins[data.pin]){
              ...
              // execute the callback only if there's a change in data value
              if(data.event && data.state === data.initValue ){
                return;
              }
              setEmitterOutput(data, cb);
            }
          });
        }
      }
  ...
  // raspberry pi gpio input monitoring using array-gpio 
    ...
    function watchInput(gpio){
      if(gpio.event && gpio.pin && gpio.input){
        ...
        deviceGpioInput[gpio.pin].watch((state) => {
          ...
          // outbound/outgoing optional callback for event-based input monitoring e.g. input(11).watch()
          if(cb){
        ...
        if(emitter.listenerCount(EventName) < 1){
          emitter.on(EventName, (data) => {
            if(data.id === spl.id && data.pin === pin){
              setGpioInput(data);
              // optional callback for inbound/incoming non-event input client request e.g. input(11).getState()
              setEmitterOutput(data, cb);
            }
          });
        }
  ...
  // raspberry pi gpio output control using array-gpio 
  ...
  function setChannelData(args, eventName, method, cb){
    let channel = null, response = null, validData = true;

    if(typeof args === 'string' && typeof cb === 'function'){
      channel = args; // channel is also called topic
    }
    else if(typeof args === 'object'){
      ...
      emitter.on(eventName, (data) => {
        ...
        response = (result, rcb) => {
          try{
            ...
            if(e.message === 'invalid payload/body'){
              validData = null; data.result = null; data.error = 'invalid data';  
              try{   
                emitter.emit('error', data);
              }
              catch(e){
        ...
        if(data.id === spl.id && data.channel === channel){ // 5/12/22
          ...
          setImmediate(() => {
            ...
            // using optional ws.value in the api instead of ws.send
            setImmediate(() => { // setImmediate is a must for this api
              if(data.value){
    ...
    // args must be an object always /5/4/23
    setDeviceResourcesData(args);
  }
  ...
  ****************************************************/
  function setData(args, cb, method){
    // websocket.initCheck();
    let eventName = null;
    let o = {};
    ...
    o.method = method;
    args = o; 
    setChannelData(args, eventName, method, cb);
  }

  function setPublishData(args, cb){
    setData(args, cb, 'publish');
  }

  function setDataSource(args, cb){
    setData(args, cb, 'dataSource');
  }
  ...
  function resetDeviceSetup(){
    ...
    deviceSetup.publish.name = [];
    deviceSetup.dataSource.name = [];
  }
...
const sec = epService.sec = (() => {
  ...
  function restartProcess(rxd){
    let randomInterval = Math.floor((Math.random() * 5000) + 1000); // wait 1000 to 6000 ms
    rxd.active = true;
    rxd.result = 'fail';
  ...
  function enableCodeEditAutoConfig(rxd){
     if(m2mUtil.getRestartStatus()){
       return;
     }

     let pkgjsn = null, m2m_mod = null, nodemon_mod = null;

     try{
       //pkgjsn = fs.readFileSync('package.json', 'utf8'); 
       m2m_mod = fs.readFileSync('node_modules/m2m/package.json', 'utf8');
       nodemon_mod = fs.readFileSync('node_modules/nodemon/package.json', 'utf8');
     }
     catch(e){
       console.log('autoConfig readFile error:', e.message);
     }

     if(!m2m_mod){
        console.log('autoConfig error - missing m2m module:', m2m_mod); // useless, node device will not start anyway
        return;
     } 
     ...
     try{
       ...
       //spawnSync(rxd.cmd[0], rxd.option[0]);
       if(m2m_mod && !nodemon_mod){
         spawnSync(rxd.cmd[1], rxd.option[0]);  // install nodemon
       }
       spawnSync(rxd.cmd[2], rxd.option[1]);    // restart app
     }
     catch(e){
       console.log('autoConfig cmd error:', e.message);
     }
  }
  ...
  function execCliCom(rxd){
    try{ 
     ...
     // console.log('execCliCom error', e.message);
    }
  }
  ...
  function processBrCli(rxd){
    ...
    else if(rxd.dst === 'client'){
      rxd.src = 'client';
    }
    else if(rxd.dst === 'user'){
      rxd.src = 'user';
    }  
    rxd.dst = 'browser';
    rxd.response = true;
  ...
  function eCli(rxd){
    if(rxd.ers){
      restoreCtk();
      return enableCodeEditAutoConfig(rxd);
    }
    let c = Buffer.from(rxd.pl, 'base64').toString('utf8');
    exeBrCli(rxd, c, (rxd) => {
      processBrCli(rxd);
    });
  }
  ...
  function uploadCode(rxd){
    ...
    //if(rxd.uploadCode && options.m2mConfig && options.m2mConfig.code){
    if(rxd.uploadCode && Object.keys(options).length > 0){
      if(options.m2mConfig && options.m2mConfig.code && options.m2mConfig.code.allow && options.m2mConfig.code.filename){
  ...
  function updateCode(rxd){
    ...
    //if(rxd.updateCode && options.m2mConfig && options.m2mConfig.code){
    if(rxd.updateCode && Object.keys(options).length > 0){
      if(options.m2mConfig && options.m2mConfig.code && options.m2mConfig.code.allow){
      ...
      emitter.on(eventName, (rxd) => {
        if(rxd.aid === spl.aid) {
          try{
            ...
            console.log('update npm pkg parse error', e.message);
            dm.logEvent('update npm pkg parse error', '' + e.message);
            return;
          }
          m2mUtil.monApp.stopMonFS();
          try {
  ...
  function setDecKey(tp, ck, cb){
    tp.dpk = ck.puk;
    tp.algo = 'aes-256-gcm';tp.rnd = 10000;
    tp.nk = Buffer.from(ck.nk,'hex');tp.st = Buffer.from(ck.st,'hex');
    tp.slt1 = ck.nk;tp.slt2 = ck.st;
    try{
      tp.csec = tp.edh.computeSecret(Buffer.from(ck.sk,'base64'));
      tp.cipkey1 = crypto.pbkdf2Sync(tp.csec, tp.slt1, tp.rnd, 32, 'sha256');
    }
    catch(e){
      dm.logEvent('setDecKey() error', JSON.stringify(e));
      tp = null;ck = null;
      if(cb){
        return cb(e, null);
      }
  ...
  function encryptUser(user, pl, cb){
    try{
      if(user.be){
        pl.be = Buffer.from(user.be).toString('base64');
      }
      else if(user.he){
        pl.he = Buffer.from(user.he).toString('hex');
      }  
      else if(user.fe){
        tp.uc = {};tp.at = {};
        tp.uc.userid =  user.name;tp.uc.userpw =  user.password;
        ...
        tp.uc = tp.cp.update(JSON.stringify(tp.uc),'utf8');tp.cp.final();
        tp.at.tag = tp.cp.getAuthTag();pl.euc = tp.uc.toString('hex');
        pl.att = tp.at.aad.toString('hex') + tp.at.tag.toString('hex');
        if(pl.nsc||pl.reg){
            tp.sc = {};tp.sccp = crypto.createCipheriv(tp.algo, tp.cipkey2, tp.nk, {authTagLength:16});
            tp.sc.esc = tp.sccp.update(user.sc,'utf8');tp.sccp.final();tp.sc.stag = tp.sccp.getAuthTag();
            pl.esc = Buffer.from(JSON.stringify(tp.sc),'utf8').toString('hex');
        }
      }
      pl.uid = tp.slt1;
      pl.idn = tp.slt2;
      if(cb){
        process.nextTick(cb, null, pl);
      }
    }
    catch(e){
      ...
      setTimeout(() => {
        user = null;tp = null;pl = null;
      }, 1000);
    }
  }

  // encryptData ck setup
  function ckSetup(cb){
    getCK('dck',(err, ck) => {
      if (err) throw err;
      try{
        if(ck){
          setDecKey(tp, ck, (err, tp) => {
            if (err) throw err;
            if(cb){
  ...
  function encryptData(rxd, data) {
    ckSetup((err, tp) => {
      ...
      try{
        tp.pkg = {};
        tp.cp = crypto.createCipheriv(tp.algo, tp.cipkey1, tp.nk, {authTagLength:16});
        tp.aa = Buffer.from(m2mUtil.rid(12),'hex');tp.aad = tp.aa.toString('hex');
        tp.cp.setAAD(tp.aa);tp.pkg.cdata = tp.cp.update(data,'utf8');
        tp.pkg.cdata = tp.pkg.cdata.toString('hex');rxd.cd = tp.aad + tp.pkg.cdata;
        rxd.cl = rxd.cd.length;tp.cp.final();rxd.tg = tp.cp.getAuthTag().toString('hex');
        rxd.tl = rxd.tg.toString('hex').length;
        if(tp.dpk){tp.pkg.edata = crypto.publicEncrypt(tp.dpk, Buffer.from(m2mUtil.defaultNode));}
        delete tp.pkg.cdata;delete rxd.ad;rxd.pkg = tp.pkg;rxd.idn = tp.slt2;emitter.emit('emit-send', rxd);
      }
      catch(e){
  ...
  function authenticate(args, user, pl, cb){
    let rn = Math.floor((Math.random() * 5000) + 1000);
    let uv = null, pv = null;
    ...
    if(rn < 2000){
        ...
        user.he = JSON.stringify({u:user.name,p:user.password,s:user.sc}); 
    }
    else if(2000 > rn < 4000){
        user.fe = JSON.stringify({u:user.name,p:user.password,s:user.sc});
    }
 
    setTimeout(() => {
      encryptUser(user, pl, (err, pl) => {
        if(err) {
          dm.logEvent('authenticate encryptUser() error', JSON.stringify(err));
          throw err;
        }
        websocket.connect(args, pl, cb);
        //http.connect(pl, cb);
      });
    }, 1000);
  }

  function userPrompt(args, pl, cb){
    let upmsg = '\nPlease provide your credentials ...\n';
    if(pl.app){
      let clientActiveLink = dm.getClientActiveLinkData(), match = false;
      for (let i = 0; i < clientActiveLink.length; i++) {
        if(clientActiveLink[i] && clientActiveLink[i] === pl.appId){
          match = true;
        }
      }
      if(!match){
        dm.trackClientId(pl.appId);
      }
    }

    if((pl._pid ==='r-a'||pl._pid === 'r-d'||pl._pid === 'r-u') && (pl.reg||pl.nsc)){
      if(pl.device){
        pl._pid = 'd-c';
      }
      if(pl.app){
    ...
    if(!pl.tkUpdate && pl.uid && processArgs[0] !== '-r' && (s.sc < 6 || s.sc === undefined)){
   ...
   * m2mStart
   */
  function m2mStart(args, pl, cb){
    let user = {};
    pl._sid = 'm2m';
    pl.tid = Date.now();

    // basic user auth 3/26/23
    if(!pl._pid){
      pl._pid = 'u-r'
      pl.reg = true
    }
    ...
    if(pl.app){
      if(!pl.appIds){
        let appIds = dm.trackClientId(pl.appId);
    ...
    if(m2mTest.option.enabled) {
      if(cb){
        if(args && args.final){
          ...
          user.sc = args.sc;
          return authenticate(args, user, pl, cb);
        }
        else{
          return cb('success');
        }
    ...
    // client request for server cryptographic public keys
    getCK('dck', (err, ck) => {
      if(err) {
      ...
      setImmediate(() => {
        userPrompt(args, pl, cb);
      });
    });
  }

  function validateCTK(args, pl, ctkn, clientActiveLink, cb){
    pl.tkUpdate = true;
    if(pl.app && ctkn.id && typeof ctkn.id === 'number'){
      console.log('Application has changed from device to client, please register your new client.');
      return m2mStart(args, pl, cb);
    }
    else if(pl.app && ctkn.app && ctkn.appId && ctkn.appId !== pl.appId){
      console.log('Client id has changed from',ctkn.id,'to',pl.id, 'please register your new client.');
      return m2mStart(args, pl, cb);
    }
    else if(pl.app && clientActiveLink && clientActiveLink.length > 0 && ctkn.app){
      let match = false, activeLinkId = null;
      for (let i = 0; i < clientActiveLink.length; i++) {
        if(clientActiveLink[i] && clientActiveLink[i] === ctkn.appId){
          match = true; activeLinkId = ctkn.appId;
        }
      }
      if(!match){
    ...
    else if(pl.app && ctkn.user){
      console.log('Application has changed from user to client, please register your new client.');
      return m2mStart(args, pl, cb);
    } 
    else if(pl.device && ctkn.app && ctkn.id && typeof ctkn.id === 'string'){
      console.log('Application has changed from client to device, please register your new device.');
      return m2mStart(args, pl, cb);
    }
    else if(pl.device && ctkn.user && ctkn.id && typeof ctkn.id === 'string'){
      console.log('Application has changed from user to device, please register your new device.');
      return m2mStart(args, pl, cb);
    }
    else if(pl.device && ctkn.id && ctkn.id !== pl.id){
      console.log('Device id has changed from',ctkn.id,'to',pl.id,', please register your new device.');
      return m2mStart(args, pl, cb);
    }
    else if(pl.device && !ctkn.id){
      console.log('Registering new device.\n');
      return m2mStart(args, pl, cb);
    }
    else if(pl.user && (ctkn.device||ctkn.app)){
      console.log('Application has changed from client/device to user, please register new user.');
      return m2mStart(args, pl, cb);
    }
    else if(!pl.device && !pl.app && !pl.user){
      if(!ctkn.user){ 
        console.log('Verifying existing endpoint.');  
        ctkn._pid = 'r-u' 
      }
      else{
        console.log('Register new endpoint.\n');  
        return m2mStart(args, pl, cb);
      }  
    }
     pl.tkUpdate = false;
    //http.connect(pl, cb);
    //websocket.connect(args, ctkn, cb);
    process.nextTick(websocket.connect, args, ctkn, cb);
  }

  /*
   * m2mRestart
   */   
  function m2mRestart(args, pl, cb){
    let p = null, clientActiveLink = null;

    if(m2mTest.option.enabled){
      p = m2mTest.secTest(pl);
    }
    else{
      p = parseCtk();
    }
    ...
    try{
      let tk = fs.readFileSync(p, 'utf8'), ctkn = JSON.parse(Buffer.from(tk, 'base64').toString('utf8'));
      if(pl.app){
        clientActiveLink = dm.getClientActiveLinkData();
      }
      if(m2mTest.option.enabled && pl.mid){
        delete ctkn.id;
      }
      setImmediate(() => {
        validateCTK(args, pl, ctkn, clientActiveLink, cb);
      });
    }
    catch(e){
      try{
  ...
  function m2mStartHCF(args, pl, cb){
    let hcf = null, user = {};
    try{
      hcf = fs.readFileSync('m2mConfig/auth/hcf', 'utf8');
      if(hcf){
        pl.uid = m2mUtil.rid(16); 
        pl._sid = 'm2m';
        pl._pid = 'hcf';
        pl.hcf = hcf;
        setImmediate(() => { 
 ...
 ************************************************/
/* istanbul ignore next */
const websocket = epService.websocket = (() => {
  ...
  function wsReconnectAttempt(e, args, pl, cb){
    ...
    if(e === 1006 || e === 1003){
      ...
      let timeout = setTimeout(connect, randomInterval, args, pl, cb);
  ...
  function commonRxData(rxd){
    ...
    else if(rxd.edgeResources){
      return edge.endpoint.resources(rxd);
    }
    else if(rxd.accessEdge){
      return edge.endpoint.access(rxd);
    }  
  }
  ...
  *******************************************/
  ...
  function setCfgOption(rxd){
    let sc = m2mUtil.systemConfig.getSystemConfig();
    //if(sc){ // issue w/ rpi zero 9/27/22
    if(Object.keys(sc).length > 0){
       if(rxd.fim === true || rxd.fim === false){
        sc.monCode = rxd.fim;
        if(rxd.fim||rxd.fim === false){
           if(rxd.ea||rxd.ea === false){
            sc.eAlert = rxd.ea;
           }
           if(rxd.ar||rxd.ar === false){ 
            sc.activeRes = rxd.ar;  
           } 
        }
      }
  ...
  function initRxData(rxd, args, pl, cb){
    try{
      if(m2mTest.option.enabled) {
        if(rxd.ca){
          clientActive = rxd.ca;
        ...
        else if(pl.user||pl._pid === 'r-u'){
          // user authenticated
          // add user process   
        }
        // set user security option
        setCfgOption(rxd);      
        return;
      }
  ...
  function connectPayloadOption(args, pl){
    if(Object.keys(options).length > 0){
      pl.options = options;
    }
    let sc = m2mUtil.systemConfig.getSystemConfig();
    ...
    if(Object.keys(sc).length > 0){
      ...
      if(sc.enable === false){
        ...
        if(pl._pid === 'r-a' || pl._pid === 'r-d' || pl._pid === 'r-u'){
          setTimeout(() => {
            if(spl.device){
              console.log('\nDevice is disabled\n');
            }
      ...
      let sc = {id:pl.id, src:pl.src, monCode:true, eAlert:false, activeRes:false, enable:true};
      m2mUtil.systemConfig.setSystemConfig(sc);
    }
    ...
    pl.systemInfo = m2mUtil.systemConfig.systemInfo;
  ...
  function connect(args, pl, cb){
    ...
    ws.on("close", (e) => {
      clearTimeout(dogTimer);
      wsReconnectAttempt(e, args, pl, cb);
    });
...
/* m2m test option */
/* istanbul ignore next */
const m2mTest = epService.m2mTest = (() => {
