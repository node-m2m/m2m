...
const client = exports.client = (() => {
  ...
  const regexPayload = /^[A-Za-z0-9 \[\]"{}':,._-]*$/;
  ...
  function setupInfo(cb){
    ...
    if(m2mTest.option.enabled){
      if(spl.testParam === 'valid')
        emitter.emit(eventName, {_pid:pl._pid, id:pl.id, setupData:[100, 200]});
      else{
        emitter.emit(eventName, {_pid:pl._pid, id:pl.id, setupData:['100', '200'], error:spl.testParam});
      }
    }
  }
  ...
  function connect(args, m2m, cb){
    ...
    ws.on("message", (data) => {
      try{
        ...
        //console.log('invalid ws incoming data', e);
