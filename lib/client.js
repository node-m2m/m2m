...
const sec = (() => {
  ...
  const m2mStart = exports.m2mStart = function(args, m2m, cb){
    ...
    if(args && (!args.userid || !args.pw || !args.sc)){
    ...
    if(rxd.channel || rxd.name){
      setImmediate(() => {
        if(rxd.event){
          ...
          return device.getDataEvent(rxd);
        }
      });
    }
      ...
      setImmediate(() => {
        if(rxd.event){
					if(testOption.enable) {
            device.deviceGetGpioInputState(rxd);
  ...
  *******************************************/
  ...
  function connect(args, m2m, cb){
    ...
    if(m2m.device){
      ...
      //console.log('dogTimerInterval', dogTimerInterval);
