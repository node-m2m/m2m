...
const client = (() => {
  ...
  let userDevices = [], clientDeviceId = [], activeSyncGpioData = [], activeSyncChannelData = [], invalidDevices = [];
...
const sec = (() => {
  ...
  const setAttributes = exports.setAttributes = (args) => {
    try{
      if(args && ((typeof args !== 'object') || !(args instanceof Object))){
        ...
        throw new Error('invalid arguments, it must be an object');
      }
      if(args.code && typeof args.code === 'object' && args.code.allow && args.code.filename){
        if(args.code.filename.length > 30){
  ...
  const m2mStart = exports.m2mStart = function(args, m2m, cb){
    ...
    if(args && typeof args === 'object' && (!args.userid || !args.pw || !args.sc)){
			console.log('Credential option provided is incomplete ...');
    } 
    ...
    if(m2m.options){
      process.nextTick(setAttributes, m2m.options);
    }
  ...
  const m2mRestart = exports.m2mRestart = function(args, m2m, cb){
    ...
    if(testOption.enable && m2m.start){
    	...
    	path = 'node_modules/m2m/lib/sec/tk';
    }
    if(m2m.options){
      process.nextTick(setAttributes, m2m.options);
    }
  ...
  function DeviceRxData(rxd){
    ...
    if(rxd.src === 'device' || rxd.deviceResponse || rxd.device){
      ...
      return device.GetGpioOutputState(rxd);
			...
			return codeUpdate(rxd);
    }
    else if(rxd.uploadCode){
			return codeUpload(rxd);
    }
