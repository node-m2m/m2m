...
const client = exports.client = (() => {
  ...
  ****************************************************/
  ...
  // http property get data capture
  ...
  // http property post data capture w/ body as payload
  ...
  Channel.prototype = { 
    ...
    // non-event data capture properties
    getData:getData,
    sendData:sendData,
...
const device = exports.device = (() => {
  ...
  ******************************************/ 
  ...
  function setChannelData(args, eventName, cb, method){
    ...
    let channelName = null, response = null, apiMethod = null;
    ...
    if(emitter.listenerCount(eventName) < 1){
      emitter.on(eventName, (data) => {
        ...
        if(data.id === spl.id && data.name === channelName){
          ...
          if(method && apiMethod === 'post' && (!data.body || data.get)){
            data.error = 'Invalid post request, missing post body';
            return emitter.emit('emit-send', data);
          }
           if(method && apiMethod === 'get' && (data.body || data.post)){
  ...
  ****************************************************/
  ...
  function getApi(args, cb){
    websocket.initCheck();
    let o = {};let eventName = null, method = 'get';
    if((typeof args === 'string' || args instanceof String) && typeof cb === 'function'){
      o.name = args;o.api = args;
      eventName = o.name + spl.id; 
      setChannelData(o, eventName, cb, method);
    }
    else if(typeof args === 'object' || args instanceof Object){
      if(typeof args.name !== 'string'){
        throw new Error('name property parameter must be a string');
  ...
  function postApi(args, cb){
    websocket.initCheck();
    let o = {};let eventName = null, method = 'post';
    if((typeof args === 'string' || args instanceof String) && typeof cb === 'function'){
      o.name = args;o.api = args;
      eventName = o.name + spl.id; 
      setChannelData(o, eventName, cb, method);
...
const sec = exports.sec = (() => {
  ...
  const rsl = new RegExp(/\//), rbsl = new RegExp(/\\/), restartTO = 25;
  ...
  function m2mRestart(args, m2m, cb){
    try{
      ...
      if(m2m.app && data.id && typeof data.id === 'number'){
        console.log('Application has changed from device to client, please register your new client application.');
        return m2mStart(args, m2m, cb);
      }
      if(m2m.app && data.appId && data.appId !== m2m.appId){
        console.log('Client id has changed from',data.id,'to',m2m.id, 'please register your new client application.');
        return m2mStart(args, m2m, cb);
      } 
      if(m2m.app && clientActiveLink && clientActiveLink.length > 0){ 
        ...
        if(!match){
          console.log('\nClient id has changed, please register your new client application.');
          return m2mStart(args, m2m, cb);
        }
      }
      ...
      if(m2m.device && data.id && data.id !== m2m.id){
        console.log('Device id has changed from',data.id,'to',m2m.id, 'please register your new device application.');
        return m2mStart(args, m2m, cb);
      }
