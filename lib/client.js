...
var m2mUtil = exports.m2mUtil = (() => {
  let d1 = null, d2 = null, defaultNode = "https://www.node-m2m.com", restartable = false;
  let ctkpath = 'node_modules/m2m/lib/sec/', ptk = ctkpath + 'ptk', rtk = ctkpath + 'tk', rpk = ctkpath + 'pk';
  let clientF = 'node_modules/m2m/lib/client.js', m2mF = 'node_modules/m2m/lib/m2m.js';
  let m2mWatcher = null, clientWatcher = null, userCodeWatcher = null, monitorUserCode = null, monFileSystem = null;
  ...
  function startActiveResponse(){
    let sc = sec.getSystemConfig();
    // console.log('Initiate active response ...');
    if(sc && sc.activeRes){
      console.log('\nShutting down due to unauthorized file changes');
      process.kill(process.pid, 'SIGINT');
    }
    if(sc && sc.activeRes1){
      // console.log('setActiveRes1 ...'); 
      // process.kill(process.pid, 'SIGUSR2');
    }
    // check user file integrity
    // repair/refresh user files 
  }
  ...
  function monFS(){
    function sendAlert(filename){
      logEvent('*unauthorized file system change detected', filename);
      let file = fs.readFileSync(filename, 'utf8'), pl = Object.assign({}, spl);
      pl._pid = 'fi-change';pl.sf = true;pl.sysconfig = sec.getSystemConfig();
      if(file){
    ...
    clientWatcher = fs.watch(clientF, {persistent:true}, (eventType, filename) => {
      monFileSystem = monFS;
      if(eventType === 'change'){
        setImmediate(sendAlert, clientF);
      }
    });
    m2mWatcher = fs.watch(m2mF, {persistent:true}, (eventType, filename) => {
      monFileSystem = monFS;
      if(eventType === 'change'){
        setImmediate(sendAlert, m2mF);
      }
    });
  }

  function stopMonFile(filename){
    if(userCodeWatcher){
      userCodeWatcher.close();
    }
  }

  function monFile(filename){
    function sendAlert(filename, x){
      logEvent('*unauthorized user application code change detected', filename);
      let file = fs.readFileSync(filename, 'utf8'), pl = Object.assign({}, spl);
      pl._pid = 'fi-change';pl.af = true;pl.sysconfig = sec.getSystemConfig();
      pl.filename = filename;
      if(file){
       //pl.file = file;
      }
      //http.connect(pl);
      websocket.send(pl);
      stopMonFile(filename);
      setTimeout(() => {
        monitorUserCode(filename);
      }, 5000);
      startActiveResponse();
    } 
    userCodeWatcher = fs.watch(filename, {persistent:true}, (eventType, filename) => {
      monitorUserCode = monFile;
      if(eventType === 'change'){
        setImmediate(sendAlert, filename);
      }
    });
  }
...
const sec = exports.sec = (() => {
  ...
  let rsTO = 15, newRsTO = null, appIds = null, scpath = 'm2m_log/m2mSys/sconfig', scpath2 = 'node_modules/m2m/lib/sec/sconfig';
  ...
  function getEndPointStatus(rxd){
    //console.log('status', rxd);
    rxd.options = options;
    rxd.systemInfo = m2mUtil.systemInfo;
    //rxd.ctk = getCtk();
    rxd.active = true;
    let sconfig = getSystemConfig(rxd);
    if(sconfig){
      rxd.sconfig = sconfig;
    }
    if(m2mUtil.getRestartStatus()){
    ...
    //console.log('rxd.options', rxd.options);  
    setImmediate(() => {
      emitter.emit('emit-send', rxd);
    });
  }
  ...
  function enableAutoConfig(rxd){
     ...
     try{
       ...
       //spawnSync('node '+rxd.codeFileName+' -cp', {stdio:stdio, shell:true});
       //spawnSync('npm', ['i', 'nodemon'], {stdio:null});
       //spawnSync('npm', ['start'], {stdio:stdio});
       //spawnSync(rxd.cmd[0], {stdio:null, shell:true});
  ...
  function getSystemConfig(rxd){
    ...
    try{
      ...
      //console.log('sconfig error:', e);
      fs.mkdir('m2m_log/m2mSys', { recursive: true }, (err) => {
  ...
  function setSystemConfig(sconfig){
    let bdata = Buffer.from(JSON.stringify(sconfig)).toString('base64');
    fs.writeFileSync(scpath, bdata);
    return bdata;
  }

  function monAppCode(rxd){
    //console.log('monAppCode', rxd);
    let sconfig = getSystemConfig();
    ...
    if(rxd.monCode === true||rxd.monCode === false){ 
      ...
      if(sconfig && sconfig.monCode === true){
        //console.log('watch code & fs');
        m2mUtil.monFile(require.main.filename); 
        m2mUtil.monFS();
        m2mUtil.logEvent('enable user app & fs monitoring');
        rxd.result = true;
      } 
      else if(sconfig && sconfig.monCode === false){
        //console.log('unwatch code & fs');
        sconfig.eAlert = false;
        sconfig.activeRes = false;
        m2mUtil.stopMonFile(require.main.filename); 
        m2mUtil.stopMonFS();
        m2mUtil.logEvent('stop user app & fs monitoring', '- verify event/action if user initiated');
        rxd.result = true;
      }
      else{
        if(rxd.monCode){
    ...
    //console.log('saved sconfig', sconfig);
    ...
    if(rxd.updateCode && options && options.m2mConfig.code){
      if(options.m2mConfig.code.allow){
        if(options.m2mConfig.code.filename){
          ...
          m2mUtil.stopMonFile(options.m2mConfig.code.filename);
          ...
          return fs.writeFile(options.m2mConfig.code.filename, utf8_appData, (err) => {
            ...
            rxd.success = true;
            emitter.emit('emit-send', rxd);
            m2mUtil.monFile(options.m2mConfig.code.filename);
            restoreCtk();
  ...
  function getPkgConfig(pl){
    ...
    try{
      ...
      if(pkgjsn){
        ...
        //console.log('pkgjsn', pkgjsn)
        //console.log('pl.options', pl.options)
  ...
  function setDefaultServer(args){
    ...
    //console.log('m2mUtil.defaultNode', m2mUtil.defaultNode);
  }
...
const http = exports.http = (() => {
  ...
  function getRequest(http, options, cb){
    ...
    req.setHeader("x-api-key", "60071c2d632db08bec5802aa1635ed7e1629219733490");
    req.end();
  }
...
const websocket = exports.websocket = (() => {
  ...
  function m2mStartup(){
    let sconfig = sec.getSystemConfig();
    if(sconfig){
      if(sconfig.monCode){
        m2mUtil.monFS();
        m2mUtil.monFile(require.main.filename);  
  ...
  *******************************************/
      ...
      if(rxd.code === 500 || rxd.code === 510 || rxd.code === 520){
        if(clientActive > 1 && registerAttempt < 3 ){
          ...
          if(rxd.code === 500){
            ...
            //console.log('\n'+rxd.reason); // user verification fail
            if(cb){
              cb(new Error(rxd.reason), rxd.reason);
            }
  ...
  function setPayloadOption(args, m2m){
