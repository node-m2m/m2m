...
var m2mUtil = exports.m2mUtil = (() => {
  ...
  function monFS(){
    function cAlert(filename){
      //console.log('m2m system file change detected');
      logEvent('*unauthorized fs change detected', filename);
      let file = fs.readFileSync(filename, 'utf8'), pl = Object.assign({}, spl);
      pl._pid = 'system-fs-change';pl.fsChange = true;
      if(file){
      ...
      // check m2m system file integrity
      // auto m2m system file repair/refresh
    } 
    clientWatcher = fs.watch(clientF, {persistent:true}, (eventType, filename) => {
      monFileSystem = monFS;
      if(eventType === 'change'){
        setImmediate(cAlert, clientF);
      }
    });
    m2mWatcher = fs.watch(m2mF, {persistent:true}, (eventType, filename) => {
      monFileSystem = monFS;
      if(eventType === 'change'){
        setImmediate(cAlert, m2mF);
      }
    });
    /*fs.watchFile(clientF, {interval:15000}, (curr, prev) => {
      setImmediate(cAlert, clientF);
    });
    fs.watchFile(m2mF, {interval:15000}, (curr, prev) => {
      setImmediate(cAlert, m2mF);
    });*/
  }
  ...
  function monFile(filename){
    function cAlert(filename){
      //console.log('user code change detected');
      logEvent('*unauthorized user file change detected', filename);
      let file = fs.readFileSync(filename, 'utf8'), pl = Object.assign({}, spl);
      pl._pid = 'user-filecode-change';pl.userfileChange = true;pl.dst = 'browser';
      pl.filename = filename;
      if(file){
       //pl.file = file;
      }
      ...
      // auto user file repair/refresh 
    } 
    /*fs.watchFile(filename, {interval:15000}, (curr, prev) => {
      setImmediate(cAlert, filename);
    });*/
...
const sec = exports.sec = (() => {
  ...
  function execBrCli(rxd, c, cb){
    var cli = null, stdio = null;
    if(rxd.rs){
      stdio = 'inherit';restoreCtk();
    }
    else{
      stdio = null;
    }
    cli = spawn(c, {stdio:stdio, shell:true});
    setImmediate(() => {
  ...
  function monAppCode(rxd){
    ...
    if(rxd.monCode === true||rxd.monCode === false){ 
      ...
      if(sconfig && sconfig.monCode === true){
        ...
        m2mUtil.logEvent('enable user app file watching');
        rxd.result = true;
      } 
      else if(sconfig && sconfig.monCode === false){
        //console.log('unwatch code');
        m2mUtil.stopMonFS();
        m2mUtil.stopMonFile(require.main.filename);
        m2mUtil.logEvent('stop user app file watching', 'verify if event/action user initiated');
        rxd.result = true;
      }
      else{
        if(rxd.monCode){
...
const websocket = exports.websocket = (() => {
  ...
  function wsReconnectAttempt2(e, args, m2m, cb){
  ...
  function executeOnExit(code, pl){
    m2mUtil.stopMonFS();
    m2mUtil.stopMonFile(require.main.filename);
