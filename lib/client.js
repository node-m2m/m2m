      ...
      resourceAccessRateContainer.forEach((dataObject, x) => {
        ...
        if(dataObject.name && dataObject.get && dataObject.method === 'get'){
            httpGetRate.forEach((object, x) => {
              if(object.name === dataObject.name||object.name === dataObject.baseUrl){
                object.cpm++;
                object.get = true;
                if(object.cpm === 1){
                  //object.data = dataObject;
...
const websocket = exports.websocket = (() => {
  ...
  *******************************************/
  ...
  function connect(args, m2m, cb){
    ...
    /*setTimeout(() => {
      console.log('setChannelEventListener emitters', emitter.eventNames()); 
    }, 3000);*/
  ...
  function setEmitSendListener(){
    ...
    if(emitter.listenerCount(eventName) < 1){
      emitter.on(eventName, (data) => {
        ...
        /*if(data.device){
          if(device.deviceSetup){
            device.deviceSetup.channel.name.forEach((name) => {
              if(data.name === name && data.srcId === spl.id && data.dstId === spl.id){
                console.log('invalid emit-send data name');
                dm.logEvent('invalid emit-send data name', 'dstId', data.dstId);
                return;
              }
              else if(data.name === name){
                console.log('srcId', rxd.srcId, 'dstId', rxd.dstId, 'data name', data.name);
              }
            });
          }
        }

        if(data.srcId === spl.id && data.dstId === spl.id){
          //console.log('invalid emit-send data');
          dm.logEvent('invalid emit-send data', 'dstId', data.dstId);
          return;
        }
        
        if(data.src !== 'device-as-client' && data.device && data.id === spl.id){
          //console.log('invalid emit-send from device-as-client');
          dm.logEvent('invalid emit-send from device-as-client', 'dstId', data.dstId);
          return;
        }*/
