...
const client = exports.client = (() => {
  ...
  let activeTry = 0, http = false, clientChannelDataListener = null, clientInputEventListener = null, clientOutputEventListener = null, activeSyncGpioData = [], activeSyncChannelData = [];
  ...
  *************************************************/
  function setGpioInputListener(pl, cb){
    ...
    clientInputEventListener = function (data){
      //console.log('input eventId', pl.eventId, data.eventId);
    ...
    if(emitter.listenerCount(eventName) < 5){ // 1 before
    ...
    //console.log('input listener count ', emitter.listenerCount(eventName)); 
  ...
  ***************************************************/
  function setGpioOutputListener(pl, cb){
    ...
    clientOutputEventListener = function (data){
      //console.log('output eventId', pl.eventId, data.eventId);
      if(data.id === pl.id && data.pin === pl.pin && data._pid === pl._pid){
        if(cb){
          setImmediate(() => {
            if(data.error){
    ...
    //console.log('output listener count ', emitter.listenerCount(eventName));    
  ...
  ****************************************************/
  function setChannelEventListener(pl, cb){
    ...
    clientChannelDataListener = function (data) {
      //console.log('data eventId', pl.eventId, data.eventId);
    ...
    if(emitter.listenerCount(eventName) < 5){
      ...
      if(pl.event && pl.watch){
        ...
        emitter.once(eventName, clientChannelDataListener); // refresh http response everytime in web apps
      }
    }
    //console.log('data listener count ', emitter.listenerCount(eventName)); 
  ...
  *************************************/
  ...
  // gpio input/output
  // e.g. client.gpio({id:100, mode:'out', pin:33}).on()
  function gpio(args){
  ...
  /*
  // e.g. client.gpio(100, {mode:'out', pin:33}).on()
  ...
  // gpio input
  // e.g client.input(200, 13).watch(cb)
  function input(id, pin){
    validateApiDeviceId(id);
  ...
  // gpio output
  // e.g client.output(200, 33).on(cb)
  function output(id, pin){
    validateApiDeviceId(id);
  ...
  // Using Http GET test only
  // e.g. client.httpGet('/m2m/usr/http-get', (data) => {})
  function httpGet(path, cb){
    HTTP.get(path, cb);
  }

  // Using Http POST test only
  // e.g. client.httpPost('/m2m/usr/http-post', {name:'ed', age:35}, (data) => {}) 
  function httpPost(path, m2m, cb){
    HTTP.post(path, m2m, cb);
  }  
...
const device = exports.device = (() => {
  ...
  ***************************************/
  ...
  function deviceWatchChannelData(rxd){
    ...
    // remove previous watch data object eventId 12/2/2021
  ...
  ******************************************/
  ...
  function startEventWatch(){
    if(watchDeviceChannelData[0]||watchDeviceInputData[0]){
      ...
      //emitter.emit('emit-send', rxd);
      //emitter.emit('emit-connect', 'device-enable');
      sec.restoreCtk();
