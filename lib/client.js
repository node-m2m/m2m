...
var m2mUtil = exports.m2mUtil = (() => {
  ...
  function monFS(){
    function sendAlert(filename, fn){
      ...
      m2mUtil.logEvent(ctkpath + 'watchLog.txt', '*unauthorized file system access', filename);
  ...
  function monUsrApp(filename){
    ...
    function sendAlert(filename, fn){
      ...
      m2mUtil.logEvent(ctkpath + 'watchLog.txt', '*unauthorized user application code access', filename);
...
const client = exports.client = (() => {
  ...
  let activeTry = 0, clientChannelDataListener = null, clientInputEventListener = null, clientOutputEventListener = null, activeSyncGpioData = [], activeSyncChannelData = [];
  ...
  *************************************************/
  function setGpioInputListener(pl, cb){
    let eventName = pl.id +  pl._pid + pl.pin + pl.event + pl.watch;
    //let eventName = pl.id + pl.pin + pl.eventId;
  ...
  ****************************************************/
  function setChannelEventListener(pl, cb){
    ...
    let eventName = pl.id + pl.name + pl.eventId;
    ...
    if(emitter.listenerCount(eventName) < 5){
      ...
      if(pl.event && pl.watch){
        ...
        emitter.once(eventName, clientChannelDataListener); // refresh http response everytime for web app integration
      }
    }
  ...
  *************************************/
  ...
  // gpio input
  // e.g client.input(200, 13).watch(cb) or client.output({id:200, pin:33}).watch(cb)
  function input(id, pin){
  ...
  // e.g. client.watch(300, 'test-channel', cb) or client.watch({id:300, channel:'test-channel'}, cb)
...
const device = exports.device = (() => {
  ...
  let deviceInputEventnameHeader = 'gpio-Input', deviceOutputEventnameHeader ='gpio-Output', outputGpioInterval = null, arrayGpio = null;
  let scanInterval = 5000, inputPin = 0, outputPin = 0, extInputPin = 0, extOutputPin = 0, simInputPin = 0, simOutputPin = 0;
  ...
  const iterateDataEvent = exports.iterateDataEvent = function (arrayData, cb){
    arrayData.forEach((objectData) => {
      if(objectData.event && objectData.name){
        let eventName = objectData.name + objectData.id;
  ...
  /*const startWatch = exports.startWatch = function (rxd, arrayData){
    if(arrayData && arrayData.length > 0){
      for (let i = 0; i < arrayData.length; i++ ) {
        if(arrayData[i]){
          if(arrayData[i].appId === rxd.appId && arrayData[i].eventId === rxd.eventId){
        ...
        for (let i = 0; i < arrayData.length; i++ ) {
          if(arrayData[i]){
            ...
            if(arrayData[i] && rxd.pin && rxd.unwatch && arrayData[i].pin === rxd.pin && arrayData[i].id === rxd.id && arrayData[i].appId === rxd.appId ){
              ...
              // send confirmation feedback to client
              emitter.emit('emit-send', rxd);
              if(cb){
  ...
  ***************************************/
  ...
  function deviceWatchChannelData(rxd){
    ...
    // option 1
    // remove previous watch channel data object
    ...
    // option2
    // re-use existing watch channel data object
    if(watchDeviceChannelData.length > 0){
      for (let i = 0; i < watchDeviceChannelData.length; i++ ) {
        if(watchDeviceChannelData[i] && watchDeviceChannelData[i].name === rxd.name && watchDeviceChannelData[i].appId === rxd.appId){
          clearTimeout(watchDeviceChannelData[i].watchTimeout);
  ...
  ******************************************/
  ...
  function deviceWatchGpioInputState(rxd){
    ...
    // option1
    // remove previous simulation watch gpio input object eventId 12/2/2021    
    ...
    // option2
    // re-use existing watch gpio input data object
    if(watchDeviceInputData.length > 0){
      for (let i = 0; i < watchDeviceInputData.length; i++ ) {
        if(watchDeviceInputData[i] && watchDeviceInputData[i].pin === rxd.pin && watchDeviceInputData[i].appId === rxd.appId){
          clearTimeout(watchDeviceInputData[i].watchTimeout);
  ...
  ******************************************/
  ...
  function deviceWatchGpioOutputState(rxd){
    ...
    // option1
    // remove previous simulation watch gpio output object eventId 12/2/2021    
    ...
    // option2
    // re-use existing watch gpio output data object
    if(watchDeviceOutputData.length > 0){
      for (let i = 0; i < watchDeviceOutputData.length; i++ ) {
        if(watchDeviceOutputData[i] && watchDeviceOutputData[i].pin === rxd.pin && watchDeviceOutputData[i].appId === rxd.appId){
          clearTimeout(watchDeviceOutputData[i].watchTimeout);
  ...
  function initRxData(rxd, args, m2m, cb){
    try{
      ...
      if(rxd.code === 500 || rxd.code === 510 || rxd.code === 520){
        //console.log('cb param', cb.length);
        if(clientActive > 1 && registerAttempt < 3 ){
          ...
          if(cb && cb.length > 0){
            ...
            //console.log('cb param', cb.length, rxd.reason);
