...
var m2mUtil = exports.m2mUtil = (() => {
  ...
  let m2mWatcher = null, clientWatcher = null, userCodeWatcher = null, logF = 'm2mConfig/log.txt', fileWatcher = null;
  ...
  function startActiveResponse(){
    ...
    console.log('Active response initiated');
    if(sc && sc.activeRes){
      let pl = Object.assign({}, spl);
      pl._pid = 'activeResponse';pl.activeResponse = true;pl.enable = false;pl.sconfig = getSystemConfig();
      //http.connect(pl);
      websocket.send(pl);
  ...
  function monFS(){
    function sendAlert(filename){
      ...
      pl._pid = 'fi-change';pl.sf = true;pl.sconfig = getSystemConfig();
      if(file){
  ...
  function monUsrApp(filename){
    ...
    function sendAlert(filename){
      ...
      let file = fs.readFileSync(filename, 'utf8'), pl = Object.assign({}, spl);
      pl._pid = 'fi-change';pl.af = true;pl.sconfig = getSystemConfig();
      pl.filename = filename;
      if(file && pl.sconfig.usrfile){
        pl.file = file;
      }
      //http.connect(pl);
      websocket.send(pl);
...
const sec = exports.sec = (() => {
  ...
  function suspend(rxd){
    ...
    if(rxd.enable === false && enable){
      ...
      console.log('Endpoint is disabled');
    }
      ...
      console.log('Endpoint is enabled');
    }
  ...
  function uploadEventLog(rxd){
    //console.log('uploadEventLog', rxd);
    rxd.active = true;
    if(m2mTest.option.enabled && Object.keys(rxd.options).length > 0){
      options = rxd.options;
    }
  ...
  function connectPayloadOption(args, m2m){
    ...
    if(sc){
      ...
      if(sc.enable === false){
        ...
        //console.log('\nDevice is enabled', sec.getEnableStatus(), '\n');
      }
    }
    else{
      let sc = {id:m2m.id, src:m2m.src, eAlert:false, monCode:true, activeRes:false, enable:true};
      m2mUtil.setSystemConfig(sc);
    }
