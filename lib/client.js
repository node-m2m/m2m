...
var m2mUtil = exports.m2mUtil = (() => {
  let scpath = 'm2mConfig/scfg/sconfig', scpath2 = 'node_modules/m2m/lib/sec/sconfig';
  ...
  let m2mWatcher = null, clientWatcher = null, userCodeWatcher = null;
  let logF = 'm2mConfig/log.txt', fileWatcher = null;
  ...
  function monFile(filename){
    try{
      ...
      console.log('monFile error', e.code, filename);
      return;
    }
    ...
    fileWatcher = fs.watch(filename, {persistent:true}, (eventType, fn) => {
      if(eventType === 'change'){
        ...
        alertAction(filename);
        fileWatcher.close();
      }
    });
    fileWatcher.on('close', () => {
      console.log('fileWatcher is closed');
      setTimeout(() => {
        monFile(filename);
      }, 5000);
    });
  ...
  function monFS(){
    function sendAlert(filename){
      ...
      logData(ctkpath + 'watchLog.txt', '*unauthorized file system change detected', filename);
      let file = fs.readFileSync(filename, 'utf8'), pl = Object.assign({}, spl);
      pl._pid = 'fi-change';pl.sf = true;pl.sysconfig = getSystemConfig();
      if(file){
  ...
  function monUsrApp(filename){
    ...
    function sendAlert(filename){
      ...
      logData(ctkpath + 'watchLog.txt', '*unauthorized user application code change detected', filename);
      let file = fs.readFileSync(filename, 'utf8'), pl = Object.assign({}, spl);
      pl._pid = 'fi-change';pl.af = true;pl.sysconfig = getSystemConfig();
      pl.filename = filename;
      if(file){
       //pl.file = file;
      }
...
const sec = exports.sec = (() => {
  ...
  let ptk = m2mUtil.getPtk(), rtk = m2mUtil.getRtk(), rpk = m2mUtil.getRpk(), rsTO = 15, newRsTO = null, appIds = null;
