...
const dm = epService.dm = (() => {
  ...
  function monitorAccessRate(){
    ...
    let publishData = deviceResources.publish.name;        // 4/5/23 
    ...
    if(monAccessRateCounter === 1){
      initDeviceData(publishData, publishDataRate); // 4/5/23 
      ...
      clientAccessData.forEach((args, i) => {   
        if(args.id === item.id){
          if(args.getData && args.name){
            //item.clientRead.push(args.name);
            //item.clientRead = m2mUtil.removeDuplicateInArray(item.clientRead);
...
const device = epService.device = (() => {
  ...
  ******************************************/
  ...
  function setDeviceResourcesData(args){
    process.nextTick(function(){
      // args will be always an object 4/5/23
      if(typeof args === 'string'){
        if(args !== 'input' && args !== 'in' && args !== 'output' && args !== 'out' && args !== '' && args !== null && args !== undefined ){
          if(args !== 'sec-cli'){
      ...
      emitter.on(eventName, (data) => {
        ...
        if(data.id === spl.id && data.channel === channel){ // 5/12/22
          ...
          setImmediate(() => {
            ...
            // watch/publish  data condition
            if(data.event && data.result && data.name && data.result !== data.initValue){
  			  data.initValue = data.result;
              //setDeviceResourcesWatchData(args); // set publish resources
            }
...
const sec = epService.sec = (() => {
  ...
  function userPrompt(args, pl, cb){
    ...
    if((pl._pid ==='r-a'||pl._pid === 'r-d') && (pl.reg||pl.nsc)){
      if(pl.device){
        pl._pid = 'd-c';
      }
      if(pl.app){
